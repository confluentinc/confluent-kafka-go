...
static const struct rd_kafka_group_info *
group_list_group_info_by_idx(struct rd_kafka_group_list *group_list, size_t cnt, size_t idx) {
	if (idx >= cnt)
		return NULL;
	return &group_list->groups[idx];
}

static const struct rd_kafka_group_member_info *
group_member_info_by_idx(struct rd_kafka_group_member_info *member_infos, size_t cnt, size_t idx) {
	if (idx >= cnt)
		return NULL;
	return &member_infos[idx];
}
*/
import "C"
...
// GroupMemberInfo provides information about a member of a consumer group.
type GroupMemberInfo struct {
	// Member ID generated by the broker (consumer id)
	MemberID string
	// Client's client.id
	ClientID string
	// Client's hostname
	ClientHost string
	// Topic partitions which are assigned to this member
...
func (m GroupMemberInfo) String() string {
	return fmt.Sprintf("%s %s %s %v", m.MemberID, m.ClientID, m.ClientHost, m.MemberAssignment)
}
...
// cToGroupMemberInfos converts a C rd_kafka_group_member_info array to a Go GroupMemberInfo slice.
func (a *AdminClient) cToGroupMemberInfos(cGroupMemberInfoList *C.struct_rd_kafka_group_member_info, cCnt C.size_t) (result []GroupMemberInfo, err error) {
	if cGroupMemberInfoList == nil || cCnt == 0 {
		return nil, nil
	}

	result = make([]GroupMemberInfo, int(cCnt))

	for i := 0; i < int(cCnt); i++ {
		cGroupMemberInfo := C.group_member_info_by_idx(cGroupMemberInfoList, cCnt, C.size_t(i))

		// Member assignment may be nil in case group is rebalancing.
		var memberAssignment []TopicPartition
		if cGroupMemberInfo.member_assignment_toppars != nil {
			memberAssignment = newTopicPartitionsFromCparts(cGroupMemberInfo.member_assignment_toppars)
		}

		result[i] = GroupMemberInfo{
			MemberID:         C.GoString(cGroupMemberInfo.member_id),
			ClientID:         C.GoString(cGroupMemberInfo.client_id),
			ClientHost:       C.GoString(cGroupMemberInfo.client_host),
...
// cToGroupInfos converts a C rd_kafka_group_list to a Go GroupInfo slice.
// Elements of rd_kafka_group_list contain only the group name and error, or the entire group information
// depending on how it was created. The argument `groupNamesOnly` should be set accordingly.
func (a *AdminClient) cToGroupInfos(cGroupInfoList *C.struct_rd_kafka_group_list, groupNamesOnly bool) (result []GroupInfo, err error) {
	if cGroupInfoList == nil {
		return nil, nil
	}

	count := int(cGroupInfoList.group_cnt)
	result = make([]GroupInfo, count)
	...
	for i := 0; i < count; i++ {
		...
		result[i].Broker = BrokerMetadata{
			...
			Host: C.GoString(cGroupInfo.broker.host),
			Port: int(cGroupInfo.broker.port),
		}

		memberCount := C.size_t(cGroupInfo.member_cnt)
		if result[i].Members, err = a.cToGroupMemberInfos(cGroupInfo.members, memberCount); err != nil {
			return nil, err
		}
	}
...
func (a *AdminClient) ListConsumerGroups(options ...ListConsumerGroupsOption) (result []GroupInfo, err error) {
	...
	if err != nil && err.(Error).Code() != ErrPartial {
		return nil, err
	}
	defer C.rd_kafka_group_list_destroy(cGroupInfoList)

	// Parse GroupResult from the resultant array .
	// Use a different error variable to retain the error in case of ErrPartial.
	result, err_parsing := a.cToGroupInfos(cGroupInfoList, true /* groupNamesOnly */)
	if err_parsing != nil {
		return nil, err_parsing
	}

	return result, err
}

// DescribeConsumerGroups describes client groups in cluster.
//
// Parameters:
...
func (a *AdminClient) DescribeConsumerGroups(groups []string, options ...DescribeConsumerGroupsOption) (result []GroupInfo, err error) {
	...
	// Parse GroupResult from the resultant array.
	// Use a different error variable to retain the error in case of ErrPartial.
	result, err_parsing := a.cToGroupInfos(cGroupInfoList, false /* groupNamesOnly */)
	if err_parsing != nil {
		return nil, err_parsing
	}
