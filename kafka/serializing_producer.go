...
func NewSerializingProducer[K, V any](conf *ConfigMap,
	...
	needsClusterID := keySerializer != nil && keySerializer.NeedsClusterID() ||
		valueSerializer != nil && valueSerializer.NeedsClusterID()
	if needsClusterID {
		...
		if keySerializer != nil && keySerializer.NeedsClusterID() {
			keySerializer.SetClusterID(clusterID)
		}
		if valueSerializer != nil && valueSerializer.NeedsClusterID() {
