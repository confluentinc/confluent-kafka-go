...
func main() {
	if len(os.Args) < 5 {
		fmt.Fprintf(os.Stderr, "Usage: %s <bootstrap-servers> <group> "+
			"<protocol=EAGER|COOPERATIVE - same for all consumers> <topics..>\n",
			os.Args[0])
		os.Exit(1)
	}
	...
	group := os.Args[2]
	protocol := strings.ToUpper(os.Args[3])
	topics := os.Args[4:]
	...
	default:
		fmt.Fprintf(os.Stderr,
			"protocol should be EAGER or COOPERATIVE, was %s\n", protocol)
		os.Exit(1)
	}
	...
	if err = c.SubscribeTopics(topics, rebalanceCallback); err != nil {
		fmt.Fprintf(os.Stderr, "Failed to subscribe to %v: %s\n", topics, err)
		os.Exit(1)
	}
	...
	for run {
		...
		default:
			...
			if err = processEvent(c, ev); err != nil {
				fmt.Fprintf(
					os.Stderr, "Failed to process event: %v: %s\n", ev, err)
			}
...
//
// Note: as a reader of this example, it is likely that all your consumers will
// use the same rebalance protocol (either EAGER or COOPERATIVE). In that case,
// you only need to include code from that particular callback, and ignore the
// other one.
func rebalanceCallback(c *kafka.Consumer, event kafka.Event) error {
	protocol := c.GetRebalanceProtocol()
	if protocol == "EAGER" {
		return rebalanceCallbackEager(c, event)
	} else if protocol == "COOPERATIVE" {
		return rebalanceCallbackCooperative(c, event)
	}
	return fmt.Errorf("unexpected rebalance protocol %s", protocol)
}

// Implementation of rebalanceCallback() for an EAGER assignment strategy, like
// range or roundrobin.
func rebalanceCallbackEager(c *kafka.Consumer, event kafka.Event) error {
	...
	case kafka.AssignedPartitions:
		...
		// the case we don't, the consumer takes care of it.
...
// Implementation of rebalanceCallback() for an COOPERATIVE assignment strategy,
// like cooperative-sticky.
func rebalanceCallbackCooperative(c *kafka.Consumer, event kafka.Event) error {
	switch ev := event.(type) {
	case kafka.AssignedPartitions:
		fmt.Printf("%% %s rebalance: %d new partition(s) assigned: %v\n",
			c.GetRebalanceProtocol(), len(ev.Partitions), ev.Partitions)
		...
		// The application may update the start .Offset of each assigned
		// partition and then call IncrementalAssign(). It is optional to call
		// IncrementalAssign in case the application is not modifying any start
		// .Offsets. In the case we don't, the consumer takes care of it.
		// It is called here despite not modifying any .Offsets for illustrative
		// purposes.
	...
	case kafka.RevokedPartitions:
		...
		// Similar to IncrementalAssign(), client automatically calls
		// IncrementalUnassign() unless the callback has already called that
		// method. Here, we don't call it.
